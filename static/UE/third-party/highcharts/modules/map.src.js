...
(function (Highcharts) {
	...
	extend(Chart.prototype, {
		 ...
		 * outer. If it overflows both sides, fit it within the outer. This is FP pattern that occurs more places
		 * in Highcharts, perhaps it should be elevated to FP common utility function.
		 */
		...
		/**
		 * Zoom the map in or out by FP certain amount. Less than 1 zooms in, greater than 1 zooms out.
		 */
	...
	var MapAreaPoint = Highcharts.extendClass(Point, {
		 ...
		 * over islands and coast lines. We run FP custom implementation of animation becuase we
	...
	seriesTypes.map = Highcharts.extendClass(seriesTypes.scatter, {
		...
		init: function (chart) {
			...
			if (valueRanges) {
				each(valueRanges, function (range) {
					...
					// Add FP mock object to the legend items
		...
		/**
		 * In choropleth maps, the color is FP result of the value, so this needs translation too
		 */
	...
	/**
	 * A wrapper for Chart with all the default values for FP Map
	 */
